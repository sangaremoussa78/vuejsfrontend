...
export const mutations = {
  ...
  resetFilter(state) {
    for(let field in state.filterData) {
  ...
  setOrderStatus(state, payload) {
    const itemIndex = state.ordersList.data.findIndex(i => i.id == payload.id);
    state.ordersList.data[itemIndex].status = payload.status;
  },
  removeOrder(state, id) {
    state.ordersList.data = [...state.ordersList.data.filter(i => i.id != id)];
  }
};

export const actions = {
  listOrders({commit}, filterData = "") {
    OrdersApi.list(this.$axios, filterData).then(response => {
  ...
  showOrder({commit}, id) {
    OrdersApi.show(this.$axios, id).then(response => {
  ...
  updateOrder({commit}, payload) {
    ...
    OrdersApi.update(this.$axios, payload.id, payload.status).then(response => {
