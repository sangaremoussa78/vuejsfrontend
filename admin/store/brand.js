...
export const actions = {
  create({commit, dispatch, state}, options) {
    ...
    BrandApi.create(this.$axios, payload).then(response => {
  ...
  listBrands({commit}, filterData = null) {
    BrandApi.list(this.$axios, filterData).then(response => {
  ...
  getAllBrands({commit}) {
    BrandApi.getAll(this.$axios).then(response => {
  ...
  deleteBrand({commit, state}, id) {
    ...
    BrandApi.delete(this.$axios, id).then(response => {
      commit('shared/setStatusMessageParameter', {key: 'showLoading', val: false}, {root: true});
      if(response.success) {
  ...
  showBrand({commit}, payload) {
    ...
    BrandApi.show(this.$axios, payload.id).then(response => {
      commit('shared/setStatusMessageParameter', {key: 'showLoading', val: false}, {root: true});
  ...
  updateBrand({commit, dispatch, state}, options) {
    ...
    BrandApi.update(this.$axios, payload, state.brand.id).then(response => {
      commit('shared/setStatusMessageParameter', {key: 'showLoading', val: false}, {root: true});
      if(response.success) {
        commit('shared/setStatusMessageParameter', {key: 'success_message', val: response.message}, {root: true});
