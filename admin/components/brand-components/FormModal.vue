    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">{{ this.$store.state.brand.brand.id == ''?'Create Brand' : 'Edit Brand #' + this.$store.state.brand.brand.id }}</h5>
        ...
        <div class="modal-body">
          ...
          <form method="post" id="brand-form" @submit.prevent="submit()">
        ...
        <div class="modal-footer">
          <button type="submit" form="brand-form" class="btn btn-primary">Save</button>
      ...
      methods: {
          ...
          submit() {
            if(this.$store.state.brand.brand.id === '') {
              this.$store.dispatch('brand/create', {onSuccess: () => this.dismiss()});
            } else {
              this.$store.dispatch('brand/updateBrand', {onSuccess: () => this.dismiss()});
            }
          }
        }
    }
