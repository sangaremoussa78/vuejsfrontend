            ...
            <tbody>
              <template v-for="order in myOrders">
                <tr :key="order.id">
                  ...
                  <td><a href="#" class="btn btn-primary btn-sm" title="View details" @click.prevent="toggleDetails(order)"><i class="fa fa-chevron-down"></i></a></td>
                </tr>
                <tr class="detail-row" v-if="order.show_details">
                  <td colspan="7">
                    <div class="order-items">
                      <h4 style="color: #6a6a57">Products</h4>
                      <ul>
                        <li v-for="orderDetails in order.order_details" style="border-bottom: 1px solid #fff">
                          <img :src="orderDetails.product.gallery[0].image_url.product_gallery_slider" v-bind:alt="orderDetails.product.title">
